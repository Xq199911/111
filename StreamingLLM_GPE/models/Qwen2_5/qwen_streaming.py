...
# Fixed by User (2025): Full Code - Fixed NameError & Argument Errors - H2O/Streaming Support
        ...
        # =========================================================================
            ...
            if generate_mode == 'batch':
                ...
                if ReadAction:
                    ...
                    # [FIXED] Variable name typo corrected: source_key_value -> source_key_values
        ...
        if is_training:
            ...
            if generate_mode == 'batch':
                ...
                if ReadAction:
                    ...
                    # [CRITICAL FIX] Generate causal mask for suffix/generation
                    # Inspect signature to support different transformers versions
                    ...
                    # We must slice the attention_mask to match the current processing length
                    # current processing length = total_past_length + current input length
                    current_len = total_past_length + inputs_embeds.shape[1]

                    if attention_mask is not None:
                        # attention_mask is (batch, full_seq_len), e.g. (1, 1867)
                        # We only need it up to current_len, e.g. (1, 61)
                        if attention_mask.shape[-1] >= current_len:
                            attention_mask_sliced = attention_mask[:, :current_len]
                        else:
                            # Fallback if mask is shorter (unlikely in this setup)
                            attention_mask_sliced = attention_mask
                    else:
                        attention_mask_sliced = None

                    # Dynamic argument handling for _prepare_4d_causal_attention_mask
                    import inspect
