...
# # Fixed by User (2025): Full Code - No Placeholders - H2O/Streaming Support
#
# import os
# import sys
# import inspect
...
#
# # -----------------------------------------------------------------------------
# # Helper Function (Locally Defined to avoid TypeError)
# # -----------------------------------------------------------------------------
#
# def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
#     """
#     Applies Rotary Position Embedding to the query and key tensors.
#     Arguments `position_ids` is optional here to support different calling conventions.
#     """
...
#
# # -----------------------------------------------------------------------------
# # Attention Module (Fully Implemented)
# # -----------------------------------------------------------------------------
#
...
#         # If we have a source cache or explicit ReadAction, use streaming path
#         if generate_mode == 'batch' and (source_key_value is not None or ReadAction):
...
#
#             # -----------------------------------------------------------
#             # Streaming Inference (Fixed Logic)
#             # -----------------------------------------------------------
...
#
#                 # Use local apply_rotary_pos_emb
#                 cos, sin = position_embeddings
...
#                     key_states, value_states = source_key_value.update(
...
#                     )
#
#                     # [SAFE RETRIEVAL OF SOURCE CACHE]
#                     if hasattr(source_key_value, 'key_cache'):
#                         # Check if cache is populated at this layer
#                         if len(source_key_value.key_cache) <= self.layer_idx:
...
#                         else:
#                             source_key = source_key_value.key_cache[self.layer_idx]
#                             source_value = source_key_value.value_cache[self.layer_idx]
#                     else:
#                         source_key, source_value = source_key_value[self.layer_idx]
#
...
#
#         # [FIX: Prevent Legacy Cache Conversion]
...
#             # [FIX: Correct Position ID Calculation for Streaming]
#             # Must include source length if we are in generation phase
#             past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
#
#             if source_key_values is not None and not ReadAction:
#                 # Add source length to current position so RoPE is correct
...
#                     past_seen_tokens += source_key_values[0][0].shape[-2]
#
...
#         else:
#             if generate_mode == 'batch':
#                 history_source_length = past_key_values.get_seq_length() if past_key_values is not None else 0
...
#                 return_dict=None, cache_position=None, training_mode='streaming', generate_mode='batch',
#                 split_mode='word', is_training=False, source_key_values=None, ReadAction=False,
#                 remove_last_token=False, _lengths=None, _lengths_index=None, attn_mask_index=None, wait_k=None,
#                 **kwargs):
#
...
#             training_mode=training_mode, generate_mode=generate_mode, split_mode=split_mode,
#             is_training=is_training, source_key_values=source_key_values, ReadAction=ReadAction,
#             remove_last_token=remove_last_token, _lengths=_lengths, wait_k=wait_k,
#         )
#
...
# coding=utf-8
# Modified 2025 by Junlong Tong (Shanghai Jiao Tong University & Eastern Institute of Technology).
# Fixed by User (2025): Full Code - Fixed Duplicate Argument Error - H2O/Streaming Support
...
# -----------------------------------------------------------------------------
# Helper Function (Locally Defined to avoid TypeError in different transformers versions)
# -----------------------------------------------------------------------------
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    """
    Applies Rotary Position Embedding to the query and key tensors.
...
class Qwen2Attention_stream(Qwen2Attention):
    ...
    def forward(
        ...
        # [FIX] Use 'source_key_values' (PLURAL)
...
class Qwen2DecoderLayer_stream(Qwen2DecoderLayer):
    ...
    def forward(
        ...
        # [FIX] Extract and POP 'source_key_values' from kwargs to avoid duplicate argument error
        ...
        attn_outputs, self_attn_weights, source_key_values, past_key_value = self.self_attn(
            ...
            cache_position=cache_position,
            position_embeddings=position_embeddings,
            source_key_values=source_key_values,  # Pass extracted value
            **kwargs,  # Pass remaining kwargs (without source_key_values)
        )
        hidden_states = residual + attn_outputs
...
class Qwen2Model_stream(Qwen2Model):
    ...
    def forward(
        ...
        # [FIX] Force accept Custom Cache (H2OCache)
        if use_cache and past_key_values is not None and not self.training:
            if hasattr(past_key_values, 'update'):
                use_legacy_cache = False
            elif not isinstance(past_key_values, Cache):
